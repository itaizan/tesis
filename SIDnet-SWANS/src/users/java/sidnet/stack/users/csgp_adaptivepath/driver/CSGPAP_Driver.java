...
import sidnet.stack.users.csgp_adaptivepath.routing.CSGP_Routing;
...
import sidnet.stack.users.csgp_adaptivepath.routing.CSGPAP_Routing;
...
public class CSGPAP_Driver {
    ...
    public static String sgp,csgp,randomCH,csgpadaptivepath;
...
/**this is the entry point in the program */
    ...
    csgpadaptivepath = args[3];
    csgp = args[4];
    sgp = args[5];
    randomCH = args[6];
    
    if(csgpadaptivepath.equals("true")&&csgp.equals("false")&&sgp.equals("true")&&randomCH.equals("false")){
        System.out.println("Clustered Shortest Geopath Routing with Adaptive Path::::CSGP-AP");
   }else if(csgpadaptivepath.equals("False")&&csgp.equals("true")&&sgp.equals("false")&&randomCH.equals("false")){
        System.out.println("Clustered Shortest Geopath Routing using non adaptive sending::::C-SGP non Adaptive");
   }else if(csgpadaptivepath.equals("False")&&csgp.equals("False")&&sgp.equals("true")&&randomCH.equals("false")){
        System.out.println("Shortest Geopath Routing::::SGP");
   }else if(csgpadaptivepath.equals("true")&&csgp.equals("false")&&sgp.equals("false")&&randomCH.equals("true")){
        System.out.println("Clustered Shortest Geopath Adaptive Path using Random Cluster Head::::CSGP-AP Random");
   }
    ...
    System.out.println("Avarage Density = "+f.computeDensity()*1000*1000+"km^2");
    System.out.println("Avarage Sesing = "+f.computeAvgConnectivity(true));
    System.out.println("Avarage Receive = "+f.computeAvgConnectivity(false));
 ...
 * @return simulation field **/
    ...
    /**Launch the SIDNet Main Graphical Interface and set-up the title*/
        ...
        if(csgpadaptivepath.equals("true")&&csgp.equals("false")&&sgp.equals("true")&&randomCH.equals("false")){
        simGUI.appendTitle("Clustered Shortest Geopath Routing with Adaptive Path::::CSGP-AP");
       }else if(csgpadaptivepath.equals("False")&&csgp.equals("true")&&sgp.equals("false")&&randomCH.equals("false")){
        simGUI.appendTitle("Clustered Shortest Geopath Routing using non adaptive sending::::C-SGP non Adaptive");
       }else if(csgpadaptivepath.equals("False")&&csgp.equals("False")&&sgp.equals("true")&&randomCH.equals("false")){
        simGUI.appendTitle("Shortest Geopath Routing::::SGP");
       }else if(csgpadaptivepath.equals("true")&&csgp.equals("false")&&sgp.equals("false")&&randomCH.equals("true")){
        simGUI.appendTitle("Clustered Shortest Geopath Adaptive Path using Random Cluster Head::::CSGP-AP Random");
       }
...
/**StatsCollector Hook-up to allow you to see a quick stats including elapsed time, number of packet lost, and so on. also used to perform run time loging*/
    ...
    statistics.monitor(new StatEntry_PacketSentContor("First_Priority...coba saja bisa sesuai kebutuhan"));
    ...
    statistics.monitor(new StatEntry_PacketDeliveryLatency("Second_Priority", StatEntry_PacketDeliveryLatency.MODE.AVG));statistics.monitor(new StatEntry_PacketSentContor("First_Priority...coba saja bisa sesuai kebutuhan"));
...
//Mengumpulkan statistic
    ...
    statistics.monitor(new StatEntry_EventDetectedContor("ALL"));
    
 
//Create the sensor nodes (each at a time).
    if(CSGPAP_Driver.csgpadaptivepath.equals("true")&&CSGPAP_Driver.csgp.equals("false")){
        for(int i=0; i<nodes-1; i++){
            myNode[i] = createNodeCSGP(i, field, place, protMap, radioInfoShared, pl, pl, simGUI.getSensorsPanelContext(), fieldContext, simManager, statistics, topologyGUI,formedCluster,sequenceNumberGlobal);
       //System.out.println("Node "+myNode[i].getID()+"Terbentuk dilokasi X:"+myNode[i].getLocation2D().getX()+"Y: "+myNode[i].getLocation2D().getY());
        }
            myNode[nodes-1]=createSinkCSGPAP(nodes-1, field, place, protMap, radioInfoShared, pl, pl, simGUI.getSensorsPanelContext(), fieldContext, simManager, statistics, topologyGUI,formedCluster,sequenceNumberGlobal);
    ...
    else if(CSGPAP_Driver.csgpadaptivepath.equals("false")&&CSGPAP_Driver.csgp.equals("true")){
        for(int i=0; i<nodes-1; i++){
            myNode[i]=createNodeCSGP(i, field, place, protMap,radioInfoShared, pl, pl, simGUI.getSensorsPanelContext(),fieldContext, simManager, statistics,  topologyGUI,formedCluster,sequenceNumberGlobal);
        }
            myNode[nodes-1]=createSinkCSGP(nodes-1, field, place, protMap, radioInfoShared, pl, pl, simGUI.getSensorsPanelContext(), fieldContext, simManager, statistics, topologyGUI,formedCluster,sequenceNumberGlobal);
                System.out.println("Sink node created at node "+myNode[nodes-1].getID()+"x loc:"+myNode[nodes-1].getLocation2D().getX()+"y loc: "+myNode[nodes-1].getLocation2D().getY());
        }
...
//menentukan jarak node ke cluster head
        ...
        float R=60; //harusnya 85 atau?? tergantung konfigurasi simulator
        double Rcx = R;
        double Rcy = 0.857*R;
        int r,c;
 ...
 */
    ...
    */
    ...
    //Routing Protocol Configuration
        ...
        CSGP_Routing chRouting=new CSGP_Routing(node);
...
//modif - ww
    public static Node createNodeCSGPAP(int id,
                                  Field field, 
                                  Placement place, 
                                  Mapper protMap, 
                                  RadioInfo.RadioInfoShared radioInfoShared,
    ...
    */
    ...
    //Routing Protocol Configuration
       ...
       // CSGPAP_Routing chRouting=new CSGPAP_Routing(node);
        CSGPAP_Routing csgpAP=new CSGPAP_Routing(node);
...
/* Here we actually start this nodes aplication layer execution. it is important to observe
    ...
    */
    ...
    //hooking up the ISO Layers, APP <--routing hookup
        csgp.setAppInterface(app.getAppProxy());
    ...
    //net <-> routing hookup
        heartbeatProtocol.setNetEntity(net.getProxy());
        csgp.setNetEntity(net.getProxy());
        
        net.setProtocolHandler(Constants.NET_PROTOCOL_INDEX_1, csgp.getProxy());
        net.setProtocolHandler(Constants.NET_PROTOCOL_HEARTBEAT, heartbeatProtocol.getProxy());
...
/* Here we actually start this nodes aplication layer execution. it is important to observe
     ...
     * @return 
     */
     public static Node createSinkCSGPAP(int id,
                                  Field field, 
                                  Placement place, 
                                  Mapper protMap, 
                                  RadioInfo.RadioInfoShared radioInfoShared,
    ...
    */
    ...
    //Routing Protocol Configuration
        HeartbeatProtocol heartbeatProtocol = new HeartbeatProtocol(net.getAddress(), node, hostPanelContext, 30*Constants.MINUTE);
        CSGP_Routing ClusterHeadRouting=new CSGP_Routing(node);
...
/* Here we actually start this nodes aplication layer execution. it is important to observe
    ...
    public static Node createSinkCSGP(int id,
                                  Field field, 
                                  Placement place, 
                                  Mapper protMap, 
                                  RadioInfo.RadioInfoShared radioInfoShared,
    ...
    */
    ...
    //Routing Protocol Configuration
        HeartbeatProtocol heartbeatProtocol = new HeartbeatProtocol(net.getAddress(), node, hostPanelContext, 30*Constants.MINUTE);
        Csgp clusterHeadRoutingCsgp=new Csgp(node);
    ...
    //hooking up the ISO Layers, APP <--routing hookup
        clusterHeadRoutingCsgp.setAppInterface(app.getAppProxy());
    ...
    //net <-> routing hookup
        heartbeatProtocol.setNetEntity(net.getProxy());
        clusterHeadRoutingCsgp.setNetEntity(net.getProxy());
        
        net.setProtocolHandler(Constants.NET_PROTOCOL_INDEX_1, clusterHeadRoutingCsgp.getProxy());
        net.setProtocolHandler(Constants.NET_PROTOCOL_HEARTBEAT, heartbeatProtocol.getProxy());
    //net-MAC-phy hookup
        byte intID=net.addInterface(mac.getProxy());
        mac.setNetEntity(net.getProxy(), intID);
