...
public class CSGPAP_App implements AppInterface, CallbackInterface {
 ...
 //String energyFile = "D:\\Dropbox\\Third_Priority KULIAH\\thesis\\SIDnet-SWANS-v1.5.6\\SIDnet-SWANS\\SIDnet-SWANS\\src\\users\\java\\sidnet\\stack\\users\\aggregate_cluster\\experiment\\csgp.csv";
    ...
    public CSGPAP_App(Node myNode, short routingProtocolIndex, StatsCollector stats,SequenceGenerator SequenceNumberGlobal)
    {
...
//ubah disini untuk menambahkan baterai
                msg.setBattery(myNode.getEnergyManagement().getBattery().getEnergyLevel_mJ());
 ...
 //        if (JistAPI.getTime()/Constants.MINUTE>120){
    ...
    //        }
      ...
      //     myNode.getNodeGUI().colorCode.mark(colorProfileGeneric, ColorProfileAggregate.SENSE2, 5);
        ...
        if(CSGPAP_Driver.csgpadaptivepath.equals("true")&&CSGPAP_Driver.csgp.equals("false")/*&&CSGPAP_Driver.sgp.equals("false")*/){
           double avgValue = 0;
           double temp =0;
           ...
           if(sensedValue>=101 && sensedValue<=200){
           	...
           	= new CSGPWrapperMessage(msgDataValue, sinkLocation,0, JistAPI.getTime());
                ...
                long sleepMin = 0;
                long sleepMax = myNode.clusterNeighboursList.size()/2;
                Random s = new Random();
        ...
        }else if(CSGPAP_Driver.csgpadaptivepath.equals("false")&&CSGPAP_Driver.csgp.equals("true")/*&&CSGPDriver.sgp.equals("false")*/){
            double avgValue = 0;
           double temp =0;
            ...
            //   myNode.msgSpace.add(sensedValue);
              ...
              // avgValue=temp/myNode.msgSpace.size();
           	...
           	= new CSGPWrapperMessage(msgDataValue, sinkLocation,0, JistAPI.getTime());
                msgAgwSensing.messageID = "F:"+this.myNode.getIP().toString()+":"+this.sequenceSensing++;
                long sleepMin = 0;
                long sleepMax = myNode.neighboursList.size()/2;
                Random s = new Random();
                long sleeptime = (long) (sleepMin + (sleepMax - sleepMin) * s.nextDouble());
  ...
  //             myNode.msgSpace.clear();
    ...
    //             myNode.msgSpace.clear();
      ...
      //         myNode.msgSpace.clear();
          ...
          }else if(CSGPAP_Driver.csgpadaptivepath.equals("true")&&CSGPAP_Driver.csgp.equals("false")&&CSGPAP_Driver.sgp.equals("true")){
            double avgValue = 0;
           double temp =0;
...
*/                netEntity.send(msgAgwSensing,
        ...
        else if(CSGPAP_Driver.csgpadaptivepath.equals("false")&&CSGPAP_Driver.csgp.equals("false")&&CSGPAP_Driver.sgp.equals("true")){
            double avgValue = 0;
           double temp =0;
             ...
             //   myNode.getNodeGUI().colorCode.mark(colorProfileGeneric, ColorProfileAggregate.SINK, ColorProfileAggregate.FOREVER);
               ...
               String messageId= "Q:"+this.myNode.getIP().toString()+":"+this.sequenceCount++;
                CSGPWrapperMessage msgCH
                	= new CSGPWrapperMessage(msgQuery, query.getRegion(),0, JistAPI.getTime());
